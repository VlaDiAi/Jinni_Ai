...
logger = logging.getLogger("JinniWebOrchestrator")

GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")  
GITHUB_REPO = os.getenv("GITHUB_REPO")    
KNOWLEDGE_DIR = "/opt/ai_orchestrator/jinni_knowledge"

app = FastAPI(title="MONOLIT-MOS AI Web Panel")
...
# Тот самый неубиваемый неоновый интерфейс пульта из ОЗУ
HTML_CODE = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
<body>
    <div class="panel">
        ...
        <div id="status" class="status">● Автономный ИИ-Рантайн Онлайн</div>
    </div>
    <script>
        ...
        textInput.onkeydown = async (e) => {
            if (e.key === 'Enter') {
                ...
                statusText.innerText = "● Автономный ИИ-Рантайн Онлайн";
...
async def process_command(request: CommandRequest):
    ...
    if "привет" in query_lower or "создатель" in query_lower or "влад" in query_lower:
        ai_reply = "Приветствую, Влад! Локальный автономный ИИ-драйвер MONOLIT-MOS полностью активен. Сетевые петли и блокировки устранены, мы работаем напрямую из памяти контейнера. Я на связи, сэр!"
        ...
        ai_reply = f"Директива '{user_query}' успешно принята ИИ-Оркестратором Джинни в автономном режиме."
