        ...
        if prefix == "SMETTER":
            ...
            if request.file_data_list and request.file_name_list:
                ...
                for idx, b64 in enumerate(request.file_data_list):
                    ...
                    if is_img and TKN:
                        c_payload = [{"type": "text", "text": "Найди замеры. Строго формат: FLOOR=число, WALL=число, PERIMETER=число"}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_data}"}}]
                        try:
                        ...
                        except Exception as e: logger.error(f"Excel RAG Error: {e}")
            ...
            return {"reply": f"Сэр, расчет выполнен!\n\n{v_ctx}\nПол = {t_fl} м², Стены = {t_wl} м²\n🚚 Объем бетона: {concrete_vol:.2f} м³.\n💰 ИТОГО: {total_sum:,.2f} руб.\n📈 МАРЖА: {margin_val:,.2f} руб.\n🚀 Смета отправлена в Telegram!", "has_estimate": True}
